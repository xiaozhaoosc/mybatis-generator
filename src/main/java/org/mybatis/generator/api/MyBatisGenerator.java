...
public class MyBatisGenerator {
    ...
    public void generate(ProgressCallback callback, Set<String> contextIds)
            throws SQLException, IOException, InterruptedException, TemplateException {
...
//            }
            writeGeneratedXmlFile(contextsToRun.get(0).getIntrospectedTables(), contextsToRun.get(0).getJavaClientGeneratorConfiguration().getTargetProject());
    ...
    public void writeGeneratedXmlFile(List<IntrospectedTable> tables, String xmlUrl) throws IOException, TemplateException {
        List<Map<String, Object>> models = new ArrayList<Map<String, Object>>();
        for (IntrospectedTable table : tables) {
            Map<String, Object> map = new HashMap<String, Object>();
            map.put("primaryColumns", table.getPrimaryKeyColumns());
            map.put("notPrimaryColumns", table.getNonPrimaryKeyColumns());
            map.put("columns", table.getAllColumns());
            map.put("sqlMapper", table.getMyBatis3SqlMapNamespace());
            map.put("model", table.getBaseRecordType());
            map.put("tableName", table.getTableConfiguration().getTableName());
            map.put("xmlName", table.getMyBatis3XmlMapperFileName());
            models.add(map);
        }

        freemarker.template.Configuration configuration = new freemarker.template.Configuration();
        configuration.setDirectoryForTemplateLoading(new File("./src/main/resource/template"));
        configuration.setDefaultEncoding("UTF-8");
        configuration.setTemplateExceptionHandler(TemplateExceptionHandler.RETHROW_HANDLER);
        Template temp = configuration.getTemplate("JavaModel.ftl");
        File file = new File(new StringBuffer(xmlUrl).append("/mapper/").toString());
        if (!file.exists())
            file.mkdir();
        for (Map<String, Object> map : models) {
            StringBuffer sb = new StringBuffer(file.getPath()).append("/");
            sb.append(map.get("xmlName"));
            Writer out = new OutputStreamWriter(new FileOutputStream(new File(sb.toString())));
